...
interface TextLayerNodeProps { isSelected: boolean; layer: TextLayer; stageHeight: number; stageWidth: number; }
function TextLayerNode({ isSelected, layer, stageHeight, stageWidth }: TextLayerNodeProps) {
...
export default function EditorCanvas({foregroundUrl,originalUrl,onExportReady}:EditorCanvasProps){
  ...
  const renderItem=(item:typeof items[number])=>item.kind==="text"?<TextLayerNode key={`text-${item.layer.id}`} layer={item.layer as TextLayer} isSelected={selectedLayer?.kind==="text"&&selectedLayer.id===item.layer.id} stageHeight={stageHeight} stageWidth={stageWidth}/>:<ImageLayerNode key={`image-${item.layer.id}`} layer={item.layer as ImageLayer} isSelected={selectedLayer?.kind==="image"&&selectedLayer.id===item.layer.id} stageHeight={stageHeight} stageWidth={stageWidth}/>;
  return <div ref={containerRef} aria-label="Image editor canvas. Select layers to drag, resize, or rotate. Double-click a layer to bring it in front of the subject." className="flex h-full min-h-[320px] w-full items-center justify-center" role="application">{originalImage&&stageWidth>0?<div className="checkerboard shrink-0 overflow-hidden rounded-2xl shadow-[0_18px_50px_rgba(0,0,0,.35)]" style={{width:stageWidth,height:stageHeight}}><Stage ref={stageRef} width={stageWidth} height={stageHeight} onMouseDown={(event)=>{if(event.target===event.target.getStage())selectLayer(null)}} onTouchStart={(event)=>{if(event.target===event.target.getStage())selectLayer(null)}}><Layer><KonvaImage image={originalImage} listening={false} width={stageWidth} height={stageHeight}/>{items.filter(item=>item.layer.placement==="behind").map(renderItem)}{foregroundImage?<KonvaImage image={foregroundImage} listening={false} width={stageWidth} height={stageHeight}/>:null}{items.filter(item=>item.layer.placement==="front").map(renderItem)}</Layer></Stage></div>:<div className="grid min-h-72 place-items-center text-sm text-[#686270]">Preparing canvas…</div>}</div>
}
